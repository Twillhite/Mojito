  ...
  scenarios: [
    {
      ...
      monthlyIncome: 6800,
      incomeTransactionOverrides: [],
      billTransactionOverrides: [],
...
async function saveScenario(input) {
  const database = await readDatabase();
  const scenario = syncAutomaticSnapshots(normalizeScenario(input));
...
function normalizeScenario(input) {
  const liabilityBalances = normalizeLiabilityBalances(input.liabilityBalances, input);
  const mortgageBalance = normalizeMortgageBalance(input.mortgageBalance, liabilityBalances);
  const autoLoanBalance = normalizeAutoLoanBalance(input.autoLoanBalance, liabilityBalances);
  ...
  return {
    ...
    homeValue: Number(input.homeValue || 0),
    homeValueGrowth: Number(input.homeValueGrowth || 0),
    mortgageBalance,
    monthlyMortgagePaydown: Number(input.monthlyMortgagePaydown || 0),
    autoValue: Number(input.autoValue || 0),
    autoValueGrowth: Number(input.autoValueGrowth || 0),
    autoLoanBalance,
    monthlyAutoLoanPaydown: Number(input.monthlyAutoLoanPaydown || 0),
    accountBalances: normalizeAccountBalances(input.accountBalances, input),
    liabilityBalances: syncSecuredDebtAccounts(liabilityBalances, {
      mortgageBalance,
      autoLoanBalance,
    }),
    ...
    incomeTransactionOverrides: normalizeTransactionOverrides(input.incomeTransactionOverrides),
    billTransactionOverrides: normalizeTransactionOverrides(input.billTransactionOverrides),
    ...
    dailySnapshots: normalizeDailySnapshots(input.dailySnapshots),
    accountSnapshots: normalizeAccountSnapshotCollection(input.accountSnapshots),
  };
}
...
function migrateScenario(scenario) {
  ...
  return {
    ...scenario,
    homeValue: Number(scenario.homeValue || 0),
    homeValueGrowth: Number(scenario.homeValueGrowth || 0),
    mortgageBalance: normalizeMortgageBalance(scenario.mortgageBalance, scenario.liabilityBalances),
    monthlyMortgagePaydown: Number(scenario.monthlyMortgagePaydown || 0),
    autoValue: Number(scenario.autoValue || 0),
    autoValueGrowth: Number(scenario.autoValueGrowth || 0),
    autoLoanBalance: normalizeAutoLoanBalance(scenario.autoLoanBalance, scenario.liabilityBalances),
    monthlyAutoLoanPaydown: Number(scenario.monthlyAutoLoanPaydown || 0),
    accountBalances: normalizeAccountBalances(scenario.accountBalances, scenario),
    liabilityBalances: syncSecuredDebtAccounts(
      normalizeLiabilityBalances(scenario.liabilityBalances, scenario),
      {
        mortgageBalance: normalizeMortgageBalance(scenario.mortgageBalance, scenario.liabilityBalances),
        autoLoanBalance: normalizeAutoLoanBalance(scenario.autoLoanBalance, scenario.liabilityBalances),
      },
    ...
    incomeTransactionOverrides: normalizeTransactionOverrides(scenario.incomeTransactionOverrides),
    billTransactionOverrides: normalizeTransactionOverrides(scenario.billTransactionOverrides),
    ...
    historicalSnapshots: Array.isArray(scenario.historicalSnapshots)
      ...
      : [],
    dailySnapshots: normalizeDailySnapshots(scenario.dailySnapshots),
    accountSnapshots: normalizeAccountSnapshotCollection(scenario.accountSnapshots),
  };
}

function normalizeAccountSnapshotCollection(input) {
  return Object.fromEntries(
    Object.entries(input || {})
      .map(([dateKey, snapshot]) => [dateKey, normalizeAccountSnapshot(snapshot)])
      .sort(([left], [right]) => left.localeCompare(right)),
  );
}

function normalizeTransactionOverrides(input) {
...
function normalizeAccountSnapshot(snapshot) {
  return {
    accountBalances: normalizeAccountBalances(snapshot?.accountBalances, {}),
    liabilityBalances: normalizeLiabilityBalances(snapshot?.liabilityBalances, {}),
  };
}

function normalizeDailySnapshots(input) {
  return (Array.isArray(input) ? input : [])
    .map((snapshot) => ({
      date: snapshot?.date || currentDateKey(),
      ...
      netWorth: Number(
        snapshot?.netWorth ??
          Number(snapshot?.cash || 0) +
            Number(snapshot?.investments || 0) +
            Number(snapshot?.retirement || 0) +
            Number(snapshot?.homeValue || 0) +
            Number(snapshot?.autoValue || 0) -
...
function buildAutomaticSnapshot(scenario) {
  const cash =
    sumBalances(scenario.accountBalances?.checking) +
    sumBalances(scenario.accountBalances?.savings);
  const investments = sumBalances(scenario.accountBalances?.investments);
  const retirement = sumBalances(scenario.accountBalances?.retirement);
  const debt =
    sumBalances(scenario.liabilityBalances?.creditCards) +
    sumBalances(scenario.liabilityBalances?.carLoans) +
    sumBalances(scenario.liabilityBalances?.mortgages) +
    sumBalances(scenario.liabilityBalances?.otherLoans);
  ...
  return {
    ...
    netWorth:
      ...
      investments +
      retirement +
      Number(scenario.homeValue || 0) +
      Number(scenario.autoValue || 0) -
      debt,
  };
}

function buildAutomaticAccountSnapshot(scenario) {
  return normalizeAccountSnapshot({
    accountBalances: scenario.accountBalances,
    liabilityBalances: scenario.liabilityBalances,
  });
}
...
function normalizeMortgageBalance(inputValue, liabilityBalances) {
  if (inputValue !== undefined && inputValue !== null && inputValue !== "") {
    return Number(inputValue || 0);
  }

  return sumBalances(liabilityBalances?.mortgages);
}

function normalizeAutoLoanBalance(inputValue, liabilityBalances) {
  if (inputValue !== undefined && inputValue !== null && inputValue !== "") {
    return Number(inputValue || 0);
  }

  return sumBalances(liabilityBalances?.carLoans);
}

function syncSecuredDebtAccounts(liabilityBalances, balances) {
  const normalized = normalizeLiabilityBalances(liabilityBalances, {});
  const existingName = normalized.mortgages?.[0]?.name || "Primary Mortgage";
  const existingAutoName = normalized.carLoans?.[0]?.name || "Primary Auto Loan";
  ...
  return {
    ...
    mortgages: [
      {
        name: existingName,
        balance: Number(balances.mortgageBalance || 0),
      },
    ],
    carLoans: [
      {
        name: existingAutoName,
        balance: Number(balances.autoLoanBalance || 0),
      },
    ],
  };
}

function syncAutomaticSnapshots(scenario) {
  const automaticSnapshot = buildAutomaticSnapshot(scenario);
  const automaticAccountSnapshot = buildAutomaticAccountSnapshot(scenario);
  const dailySnapshots = normalizeDailySnapshots(
    [
      ...(scenario.dailySnapshots || []).filter((snapshot) => snapshot.date !== automaticSnapshot.date),
      automaticSnapshot,
    ],
  );

  const monthlySnapshot = {
    month: monthKeyFromDate(automaticSnapshot.date),
    cash: automaticSnapshot.cash,
    investments: automaticSnapshot.investments,
    retirement: automaticSnapshot.retirement,
    homeValue: automaticSnapshot.homeValue,
    autoValue: automaticSnapshot.autoValue,
    debt: automaticSnapshot.debt,
  };

  const historicalSnapshots = [
    ...(Array.isArray(scenario.historicalSnapshots) ? scenario.historicalSnapshots : []).filter(
      (snapshot) => snapshot.month !== monthlySnapshot.month,
    ),
    monthlySnapshot,
  ...
  return {
    ...scenario,
    startingCash: automaticSnapshot.cash,
    startingInvestments: automaticSnapshot.investments,
    startingRetirement: automaticSnapshot.retirement,
    startingDebt: automaticSnapshot.debt,
    historicalSnapshots,
    dailySnapshots,
    accountSnapshots: normalizeAccountSnapshotCollection({
      ...(scenario.accountSnapshots || {}),
      [automaticSnapshot.date]: automaticAccountSnapshot,
    }),
  };
}
